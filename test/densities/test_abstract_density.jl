...
@testset "abstract_density" begin
    ...
    x = rand(3)
    @test_throws ArgumentError BAT.eval_logval(td, [Inf, Inf, Inf], default_dlt())
    @test_throws ErrorException BAT.eval_logval(tds, [Inf, Inf, Inf], default_dlt())
    @test_throws ErrorException BAT.eval_logval(tds, [NaN, NaN, NaN], default_dlt())
    @test_throws ArgumentError BAT.eval_logval(tds, rand(length(mvn)+1), default_dlt())
    @test_throws ArgumentError BAT.eval_logval(tds, rand(length(mvn)-1), default_dlt())

    @test @inferred(BAT.eval_logval(tds, x, default_dlt())) == @inferred(logpdf(mvn, x))
    ...
    @test @inferred(BAT.eval_logval(ud, x, default_dlt())) == -Inf
    @test @inferred(BAT.eval_logval(ud_shape_1(ud), ud_shape_1(x), default_dlt())) == @inferred(logpdf(mvu, x))
    @test @inferred(BAT.eval_logval(ud_shape_2(ud), ud_shape_2(x), default_dlt())) == @inferred(logpdf(mvu, x))

    @test_throws ArgumentError BAT.eval_logval(ud, vcat(x,x), default_dlt())

    @test BAT.eval_logval(ud, x .- eps(1.0), default_dlt()) == -Inf

    @test_throws ArgumentError @inferred(BAT.eval_logval(ud, [0 0 0], default_dlt()))
    ...
    @test_throws ArgumentError BAT.eval_logval(dd, 0, default_dlt())
    @test_throws ErrorException BAT.eval_logval(dds, 0, default_dlt())
    ...
    @test BAT.eval_logval(sd, x_for_sd_good_shape, default_dlt()) == logpdf(mvn, x1_for_sd) + logpdf(mvu, x2_for_sd)
    @test_throws ArgumentError BAT.eval_logval(sd, x_for_sd_bad_shape, default_dlt())
