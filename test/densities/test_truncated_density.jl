...
@testset "truncated_density" begin
    ...
    @test BAT.eval_logval(unshaped(BAT.truncate_density(prior, bounds)), [1, 2, 0, 3], missing) ≈ BAT.eval_logval(unshaped(prior), [1, 2, 0, 3], default_dlt())
    @test BAT.eval_logval(BAT.truncate_density(prior, bounds), varshape(prior)([1, 2, 0, 3]), missing) ≈ BAT.eval_logval(prior, varshape(prior)([1, 2, 0, 3]), default_dlt())
    ...
    @test @inferred(BAT.eval_logval(unshaped(trunc_pstr), [1, 2, 0, 3], missing)) ≈ BAT.eval_logval(unshaped(posterior), [1, 2, 0, 3], default_dlt())
    @test @inferred(BAT.eval_logval(unshaped(trunc_pstr), [-1, -1, -1, -1], default_dlt())) ≈ -Inf
