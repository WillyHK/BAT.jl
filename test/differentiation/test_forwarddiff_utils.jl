...
@testset "forwarddiff_utils" begin
    ...
    @test @inferred(BAT.forwarddiff_eval(f, 3.0, true, 4f0)) == rdf
    @test @inferred(BAT.forwarddiff_eval(fv, 3.0, true, 4f0)) == SVector(rdf)

    x = SVector(3.0, true, 4f0)
    r_ref = (ntuple(i -> DTf32(f(x)[i], ForwardDiff.jacobian(x -> SVector(f(x)[i]), SVector(3.0, true, 4f0))...), Val(2))..., 42)
