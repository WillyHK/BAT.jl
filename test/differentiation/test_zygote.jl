...
@testset "zygote" begin
    prior = BAT.example_posterior().prior.dist
...
trafo = BAT.DistributionTransform(Normal(), Weibull())
v = 0.5
@test @inferred(trafo(v)) == quantile(trafo.target_dist, cdf(trafo.source_dist, v))

BAT.forwarddiff_pullback(trafo, v)
